...
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    throw 'Certificate initialization must run elevated so the public certificate can be trusted for all local users.'
}
...
try {
    ...
    Import-Certificate -FilePath $resolved -CertStoreLocation Cert:\LocalMachine\TrustedPeople | Out-Null
    [pscustomobject]@{
        Subject = $certificate.Subject
        Thumbprint = $certificate.Thumbprint
        PublicCertificate = $resolved
        PrivateKeyExportable = $false
        TrustedForLocalMachineVerification = $true
    }
}
catch {
